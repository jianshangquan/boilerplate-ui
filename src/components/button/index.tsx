...
export function Button({ children, onClick, className = '', style, disabled = false, loading = false } : BoilerplateButtonProps){
  return (
      <button disabled={disabled || loading} style={style} onClick={onClick} className={`${disabled ? 'cursor-not-allowed' : 'cursor-pointer'} ${className} disabled:opacity-60 disabled:cursor-not-allowed border rounded-xl px-3 py-2 ${loading ? 'bg-primary' : 'bg-primary hover:bg-primary/70'} text-white dark:bg-white dark:text-black dark:hover:bg-gray-300 transition-all duration-300 flex items-center justify-center gap-2`}>
          {children}
